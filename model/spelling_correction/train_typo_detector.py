...
import numpy as np
from transformers import BertTokenizer, BertTokenizerFast, AutoModelForTokenClassification, BertForTokenClassification
...
def main(config):
    ...
    tokenizer = BertTokenizerFast.from_pretrained(config.model_source)
    ...
    model = AutoModelForTokenClassification.from_pretrained(config.model_source, return_dict=True, num_labels=2)
    model.to(device)
