...
from utils import tokenize_and_map, CHINESE_RE, tokenize_and_map_
...
def prepare_data(path, obtain_bopomofo=False, obtain_true_text=False):
    ...
    for line in open(path, encoding='utf-8').readlines():
        ...
        for (wrong_char, corrected_char, start) in content['substitute_errors']:
            if '#' in wrong_char:
                wrong_char = re.sub('#', '', wrong_char)
            if '#' in corrected_char:
                corrected_char = re.sub('#', '', corrected_char)
            # if '*' in wrong_char:
            #     wrong_char = re.sub('[*]', '[UNK]', wrong_char)
            # if '*' in corrected_char:
            #     corrected_char = re.sub('[*]', '[UNK]', corrected_char)
...
class TypoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        text = self.texts[idx]
        tokens, text2token, token2text = tokenize_and_map_(self.tokenizer, text)
